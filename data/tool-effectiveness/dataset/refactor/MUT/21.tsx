...
export const teamChannelFilterTypes = ['all', 'autoJoin'] as const;

export type TeamChannelFilterType = typeof teamChannelFilterTypes[number];
...
type TeamsChannelsProps = {
	...
	mainRoom: IRoom;
	text: string;
	type: TeamChannelFilterType;
	setType: Dispatch<SetStateAction<TeamChannelFilterType>>;
...
const TeamsChannels = ({
  ...
  const options: SelectOption[] = useMemo(
    () =>
      teamChannelFilterTypes.map((type): SelectOption => [type, t(type === 'autoJoin' ? 'Team_Auto-join' : 'All')]),
    [t]
  );
	...
	return (
		<>
			...
			<ContextualbarSection>
				...
				<Box w='x144' mis={8}>
					<Select onChange={(val) => setType(val as TeamChannelFilterType)} value={type} options={options} />
				</Box>
			</ContextualbarSection>
