...
import {
  ...
  isErrorState,
  isPendingState,
  useRemoteClusterEdit,
  useSharedChannelRemoteRows,
  useTeamOptions,
} from "./utils";
...
export default function SecureConnectionDetail(props: Props) {
  ...
  const { state: initRemoteCluster, ...location } = useLocation<
  ...
  ] = useRemoteClusterEdit(remoteId, initRemoteCluster);
  const isFormValid =
    isRemoteClusterPatch(patch) &&
    (!isCreating || Boolean(patch.display_name && patch.default_team_id));
  ...
  return (
    ...
    >
      ...
      <AdminWrapper>
        <AdminSection data-testid="connection_detail_section">
          <SectionHeader>
            ...
            {currentRemoteCluster && (
              <ConnectionStatusLabel rc={currentRemoteCluster} />
            )}
          </SectionHeader>
              ...
              <>
                ...
                <FormField
                  ...
                  helpText={formatMessage({
                    ...
                    defaultMessage:
                      "Select the default team in which any shared channels will be placed. This can be updated later for specific shared channels.",
                  })}
                >
                  <TeamSelector
      ...
      <SaveChangesPanel
        ...
        serverError={
          isErrorState(saving) || isErrorState(creating) ? (
            <FormattedMessage
        ...
        savingMessage={formatMessage({
          id: "admin.secure_connections.details.saving_changes",
          defaultMessage: "Saving secure connection…",
        })}
...
function SharedChannelRemotes(props: {
  remoteId: string;
  rc?: RemoteCluster;
}) {
  const [filter, setFilter] = useState<FromContext>();
  const [data, { loading, fetch }] = useSharedChannelRemoteRows(
    props.remoteId,
    { filter }
  );
  const { promptAdd } = useSharedChannelsAdd(props.remoteId);
  const confirmed = props.rc ? isConfirmed(props.rc) : undefined;
  const showTabs = confirmed
    ? true
    : !(confirmed === false && filter === "home" && !data);
  ...
  if (loading || !props.rc) {
    ...
    content = <Placeholder filter={filter ?? "home"} rc={props.rc} />;
  }
      ...
      <TabsWrapper>
        {showTabs && (
          <Tabs
            ...
            unmountOnExit={true}
          >
            <Tab eventKey={"remote"} title={props.rc?.display_name} />
            <Tab
              eventKey={"home"}
              title={
                <FormattedMessage
        ...
        <SectionContent $compact={Boolean(data)}>{content}</SectionContent>
      </TabsWrapper>
    </>
  );
}
  ...
  .tabs {
    ...
    .nav-tabs {
      border-bottom: 1px solid
        var(--center-channel-color-12, rgba(63, 67, 80, 0.12));
    }
  }
...
const ChannelIcon = ({ channelId }: { channelId: string }) => {
  ...
  return <ChannelIconWrapper>{icon}</ChannelIconWrapper>;
};
...
function SharedChannelRemotesTable(props: {
  ...
  const columns = useMemo<Array<ColumnDef<SharedChannelRemoteRow, any>>>(() => {
    return [
      ...
      col.accessor("team_display_name", {
        ...
        cell: ({ getValue }) => <TeamName>{getValue()}</TeamName>,
        enableHiding: false,
        enableSorting: true,
      }),
      col.display({
        id: "actions",
        cell: ({ row }) => (
          <RemoteActions remote={row.original} fetch={props.fetch} />
        ),
        enableHiding: false,
        enableSorting: false,
      }),
...
const RemoteActions = ({
  ...
  return (
    <RemoteActionsRoot>
      <LinkButton onClick={handleRemove} $destructive={true}>
        <FormattedMessage
