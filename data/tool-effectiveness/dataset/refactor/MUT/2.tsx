...
type RoutePath = LocationPathname | `${LocationPathname}?${LocationSearch}`;
...
const buildRoutePath = (to: To): RoutePath => {
	if (typeof to === 'string') {
		return to;
	}
	...
	if ('pathname' in to) {
		...
		return (pathname + encodeSearchParameters(search)) as RoutePath;
	}
	...
	if ('name' in to) {
		...
		return Tracker.nonreactive(() => FlowRouter.path(name, params, search)) as RoutePath;
	}
