...
type AgentSortField = 'name' | 'username' | 'emails.address' | 'statusLivechat';

// TODO: missing error state
const AgentsTable = () => {
	const { t } = useTranslation();

	const { sortBy, sortDirection, setSort } = useSort<AgentSortField>('name');
	const [text, setText] = useState('');
	const debouncedSort = useDebouncedValue(
		useMemo(() => [sortBy, sortDirection], [sortBy, sortDirection]),
		500,
	) as [AgentSortField, 'asc' | 'desc'];
	...
	const onHeaderClick = useEffectEvent((id: AgentSortField) => {
		if (sortBy === id) {
