...
const SearchTypeItem = styled.button<SearchTypeItemProps>`
  ...
  background-color: ${(props) =>
    props.selected ? "rgba(var(--button-bg-rgb), 0.08)" : "transparent"};
  color: ${(props) =>
    props.selected
      ? "var(--button-bg)"
...
const SearchTypeSelector = ({ searchType, setSearchType }: Props) => {
  const setMessagesSearchType = useCallback(
    () => setSearchType("messages"),
    [setSearchType]
  );
  const setFilesSearchType = useCallback(
    () => setSearchType("files"),
    [setSearchType]
  );
  ...
  return (
    <SearchTypeSelectorContainer>
      ...
      {searchPluginButtons.map(({ component, pluginId }: SearchPluginProps) => {
