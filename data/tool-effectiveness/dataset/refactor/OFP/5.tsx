...
type CustomBarProps = {
  ...
  stackKeys: unknown
  payload: unknown
  textClassName: unknown
  showPercentage: unknown
  showTopBorderRadius: unknown
  showBottomBorderRadius: unknown
};

export const CustomBar = React.memo((props: CustomBarProps) => {
  const {
    opacity,
    fill,
    x,
  ...
  const TEXT_PADDING_Y = Math.min(
    6,
    Math.abs(MIN_BAR_HEIGHT_FOR_INTERNAL_TEXT - height / 2)
  );
  ...
  const bottomBorderRadius = showBottomBorderRadius
    ? BAR_BOTTOM_BORDER_RADIUS
    : 0;
  ...
  return (
    <g>
      <path
        ...
        Q${x + width},${y + height} ${x + width - bottomBorderRadius},${
          y + height
        }
