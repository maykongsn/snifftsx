...
const Login = ({ onCustomizeHeader }: LoginProps) => {
  ...
  const getExternalLoginOptions = () => {
    ...
    if (enableSignUpWithGitLab) {
      ...
      externalLoginOptions.push({
        ...
        label:
          GitLabButtonText ||
          formatMessage({ id: "login.gitlab", defaultMessage: "GitLab" }),
    ...
    if (enableSignUpWithOpenId) {
      ...
      externalLoginOptions.push({
        ...
        label:
          OpenIdButtonText ||
          formatMessage({ id: "login.openid", defaultMessage: "Open ID" }),
  ...
  const getInputPlaceholder = () => {
    ...
    if (enableSignInWithUsername) {
      loginPlaceholders.push(
        formatMessage({ id: "login.username", defaultMessage: "Username" })
      );
    }
  ...
  const submit = async ({ loginId, password, token }: SubmitOptions) => {
    ...
    if (
      ...
      if (
        loginError.server_error_id === "api.user.login.not_verified.app_error"
      ) {
        history.push(
  ...
  const getContent = () => {
    if (showMfa) {
      return (
        <LoginMfa loginId={loginId} password={password} onSubmit={submit} />
      );
    }
    ...
    if (desktopLoginLink || query.get("server_token")) {
      return (
        <Route
          path={"/login/desktop"}
          render={() => (
            <DesktopAuthToken href={desktopLoginLink} onLogin={postSubmit} />
          )}
        />
      );
    }
        ...
        <div className="login-body-action">
          ...
          >
            <div className="login-body-card-content">
              <p className="login-body-card-title">{getCardTitle()}</p>
                ...
                >
                  <div className="login-body-card-form">
                    <Input
                      ...
                      autoFocus={true}
                      aria-describedby={
                        alertBanner ? "login-body-card-banner" : undefined
                      }
                    />
                    <PasswordInput
                      ref={passwordInput}
              ...
              {enableExternalSignup && (
                <div
                  className={classNames("login-body-card-form-login-options", {
